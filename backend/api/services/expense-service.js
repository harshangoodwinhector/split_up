...
export const save = async (req, res) => {
    ...
    // const result = await tracking.exists({ user_id: req.query.user_id});
    // if (result){
    //     console.log("in")
    //      track = await tracking.findOne({user_id: req.query.user_id})
    // }
    // else{
    //     newexp =1
    // }
    ...
    if (req.body.type === 'Group') {
       ...
       // exp.user_id = req.params.user_id;
...
export const group_expense_search = async (req,res) =>{
  ...
  //  console.log(ct);
    ...
    console.log("111");
   // let exp = expenses.filter(exp=> JSON.stringify(exp.paidby)== req.params.paidby);
   // let name = await map_name_with_id(new ObjectId(req.params.paidby));
  //let id = String(req.params.paidby);
  // let name =  await map_name_with_id(req.params.paidby);
